...
class SongPlayerPage extends StatefulWidget {
  ...
  @override
  State<SongPlayerPage> createState() => _SongPlayerPagePageState();
}

class _SongPlayerPagePageState extends State<SongPlayerPage> {
  ...
  void initState() {
    ...
    if(widget.musicVideo.sections.isEmpty){
      YoutubeMusicLinkManager.shared.getYouTubeMusicNext(widget.musicVideo);
    }else{
      _musicVideos.addAll(widget.musicVideo.sections.first.items);
    }
    ...
    if(video.videoUrl != null){
      ...
      if(mounted){
        ...
        _audioPlayer?.setAudioSource(playlist).then((duration) async {
          ...
          video.videoUrl = null;
          YoutubeMusicLinkManager.shared.links.remove(video.videoId);
        });
      }
    }
  }
  ...
  void _showPopup({bool close = true}){
    ...
    PictureInPicture.startPiP(pipWidget: SongPopupPlayer(
      ...
      originalVideo: widget.musicVideo,
      video: _musicVideo!,
    ));
    if(close){
                              ...
                              children: [
                                Expanded(
                                  child: InkWell(
                                    ...
                                    child: Text(
                                      widget.musicVideo.sections.first.title,

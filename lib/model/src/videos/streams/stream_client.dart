...
class StreamClient {
  ...
  Future<StreamManifest> getManifest(dynamic videoId,
    ...
    final watchPlayerResponse = watchPage?.playerResponse;
    if (watchPlayerResponse != null) {

      if (!watchPlayerResponse.isVideoPlayable) {
        throw {
          'videoId': videoId,
          'reason': watchPlayerResponse.playabilityStatusReason,
          'subReason': watchPlayerResponse.playabilityStatusErrorScreen
        };
      }

      String? hlsManifestUrl = watchPlayerResponse.hlsManifestUrl;
      if(hlsManifestUrl != null){
        final manifest = StreamManifest([]);
        manifest.hlsManifestUrl = hlsManifestUrl;
        return manifest;
      }
    }
      ...
      try {
        await retry(_httpClient, () async {
          final streams = await _getStreams(videoId,
                  ytClient: client, watchPage: watchPage)
              .toList();
  ...
  Future<String> getHttpLiveStreamUrl(VideoId videoId) async {
    ...
    if (!playerResponse.isVideoPlayable) {
      throw {
        'videoId': videoId,
        'reason': playerResponse.playabilityStatusReason,
        'subReason': playerResponse.playabilityStatusErrorScreen
      };
    }
  ...
  Stream<StreamInfo> _getStreams(VideoId videoId,
      ...
      required WatchPage? watchPage}) async* {
    // Use await for instead of yield* to catch exceptions
    await for (final stream
        in _getStream(videoId, ytClient, watchPage)) {
      yield stream;
    }
  }
